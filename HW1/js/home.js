...
function caricaraccolta(event) {
	...
	if (raccolta===ultimaraccoltascelta && pul===0){ // se l'ultima volta l'ho visto e riclicco lo stesso lo cancello
		hidden();                                      
		pul=1;
...
function information(json){ 
			...
			for(let i=0; i<max;i++){
			    ...
			    let ris=json[i].contenuto.split(';'); //divido il risultato del db;
